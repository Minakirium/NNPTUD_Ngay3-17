...
let { CreateUserValidator, validationResult, ChangePasswordValidator } = require('../utils/validatorHandler')
...
router.post("/changepassword", CheckLogin, ChangePasswordValidator, validationResult, async function (req, res, next) {
  try {
    let result = await userController.ChangePassword(req.user, req.body.oldpassword, req.body.newpassword);
    if (result) {
      res.send({ message: "Password changed successfully" });
    } else {
      res.status(403).send({ message: "Old password is wrong" });
    }
  } catch (err) {
